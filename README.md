...
```console
$ docker run --name some-mariadb -e MYSQL_ROOT_PASSWORD=my-secret-pw -d sorentech/rite2:tag
```
...
```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=my-secret-pw -d sorentech/rite2:tag
```
...
Start your `mariadb` container like this:
	...
	```console
		$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=my-secret-pw -d sorentech/rite2:tag
			```
			...
			Documentation for the official mariadb image is stored in the [`mariadb/` directory](https://github.com/docker-library/docs/tree/master/mariadb) of the [`docker-library/docs` GitHub repo](https://github.com/docker-library/docs). 
			
			## Issues
			
			If you have any problems with or questions about this image, please contact us through a [GitHub issue](https://github.com/SorenTech/rite2/issues).			
